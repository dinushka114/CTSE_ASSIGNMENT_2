...
module.exports.SubscribeMessageCustomer = async (channel)=>{
    try {
        await channel.assertQueue('CUSTOMER_QUEUE');
        await channel.bindQueue('CUSTOMER_QUEUE', process.env.EXCHANGE_NAME, process.env.SHOPPING_BINDING_KEY);
        return channel;
    } catch (error) {
        throw error;
    }
